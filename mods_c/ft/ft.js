...
window.bootlegger.ft.hash_file = async function(inp_file, chunk_sizemb=100)
{
	// create hash object
...
window.bootlegger.ft.queue_iterator = async function(ctrl)
{
	...
	while(true){
		...
		if (window.bootlegger.ft.file_upload_q.length <= 0 || !ctrl.alive){return}
		...
		// PROCESS FILE UPLOAD
		// FOR NOW ALWAYS LFS
		const lfs_done = await window.bootlegger.ft.lfs_upload(process_entry)
...
window.bootlegger.ft.lfs_upload = async function(inf, start_offs=0)
{
	const fl = inf['file']
