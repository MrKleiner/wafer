...
$this.set_flist_view_type = function(tp='list', shadow=false)
{
...
$this.spawn_video_unit = async function(vpath)
{
	return new Promise(async function(resolve, reject){
		...
		etype.style.backgroundImage = `url(${$this.video_bins[unit_id][int(pframe_count / 2)]})`
...
$this.vidscroll = function(evt, etgt)
{
	...
	const fr_array = $this.video_bins[pr_id]
...
$this.list_media = async function(elm='')
{
	...
	$('mpool flist').empty();
	$this.set_flist_view_type('grid', true);

	$('mpool flist').prepend(`
		<flist-entry fldpath="${window.league_match}" class="folder match" onclick="window.struct_fld = null; $this.list_match_struct(this); $this.set_flist_view_type(window.localStorage.getItem('flist_view_type'), true);">
			<etype dir_up>
			</etype>
			<ename>../</ename>
		</flist-entry>
...
$this.open_webm_preview = async function(elm)
{
	...
	if (webm_state.full != true){
		...
		if (webm_state.missing == true){
			...
			// 		<btn onclick="$this.gen_webm_manually('${flpath}')" id="manual_webm_gen">Start Generating Webm</btn>
			// 	</div>
			// `)
			$(fuckoff).replaceWith(`
				<div id="webm_still_generating">
